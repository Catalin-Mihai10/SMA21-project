...
public class AuthentificationHandler implements IAuthentificationHandler{
        ...
        databaseReference.child(Constants.CHILD_USERS).child(Constants.CHILD_USERNAME).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot dataSnapshot : snapshot.getChildren()){
                    ...
                    String hasedValueCurrentUser = HashingSHA256.getEncoding(user);
    ...
    public boolean addClientInDatabase(String user, String password) {
        if(verifyClientInDatabase(user, password)) return false;

        Log.d(Constants.AUTHENTIFICATION_HANDLE_TAG, "Adding user to database");

        databaseReference.child(Constants.CHILD_USERS).child(Constants.CHILD_USERNAME).setValue(HashingSHA256.getEncoding(user));
        databaseReference.child(Constants.CHILD_USERS).child(Constants.CHILD_PASSWORD).setValue(HashingSHA256.getEncoding(password));
    ...
    public boolean removeClientFromDatabase(String user, String password) {
        ...
        deletetingDatabaseReference = FirebaseDatabase.getInstance(Constants.DATABASE_URL).getReference().child(Constants.CHILD_USERS)
                            .child(Constants.CHILD_USERNAME).child(HashingSHA256.getEncoding(user));
        deletetingDatabaseReference.removeValue();
        deletetingDatabaseReference = FirebaseDatabase.getInstance(Constants.DATABASE_URL).getReference().child(Constants.CHILD_USERS)
                .child(Constants.CHILD_PASSWORD).child(HashingSHA256.getEncoding(password));
        deletetingDatabaseReference.removeValue();
